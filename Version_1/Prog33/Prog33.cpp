...
#include "Prog33h.h"
namespace Prog3 {
	const char *msgs[] = { "0. Quit", "1. Reorg", "2. Print one", "3. Add link", "4. Delete link" , "5. Add terminal", "6. Print all" };
	...
	/*Logical_Element Logical_Element::copy(const Logical_Element &lel) {
		Logical_Element lel1(lel);
		return lel1;
	}*/
	...
	void Logical_Element::add_terminal() {
		bool ind = true;
		while (ind) {
			std::cout << "Input index number of terminal(from 0 to " << SZ - 1 << ")-> " << std::endl;
			int i;
			while (getNum(i) < 0 || i < 0 || i > SZ - 1)
			{
				std::cout << "You're wrong. Repeat please" << std::endl;
			}
			std::cout << "Input type of terminal(0-Input,1-Output)-> " << std::endl;
			...
			if (!T) {
				if (inp[i].type == 0) {
					...
					inp[i].number = i;
					inp[i].type = T;
					std::cout << "input value of " << i << " terminal(0,1,undefined(2))-> " << std::endl;
					while (getNum(inp[i].value) < 0 || inp[i].value != 0 && inp[i].value != 1 && inp[i].value != 2)
					{
						std::cout << "You're wrong. Repeat please" << std::endl;
					}
					std::cout << "input count of links of " << i + 1 << " terminal(for Input Terminal: 0-1, for Output Termainal: 0-3)-> " << std::endl;
					while (getNum(inp[i].count) < 0 || inp[i].type == 0 && (inp[i].count < 0 || inp[i].count > 1))
					{
						std::cout << "You're wrong. Repeat please" << std::endl;
					}
					inp_n++;
				...
				if (outp[i].type == 1) {
					...
					outp[i].type = T;
					std::cout << "input value of " << i << " terminal(0,1,undefined(2))-> " << std::endl;
					while (getNum(outp[i].value) < 0 || outp[i].value != 0 && outp[i].value != 1 && outp[i].value != 2)
					{
						std::cout << "You're wrong. Repeat please" << std::endl;
					}
					std::cout << "input count of links of " << i + 1 << " terminal(for Input Terminal: 0-1, for Output Termainal: 0-3)-> " << std::endl;
					while (getNum(outp[i].count) < 0 || outp[i].type == 1 && (outp[i].count < 0 || outp[i].count > 3))
					{
						std::cout << "You're wrong. Repeat please" << std::endl;
					}
					outp_n++;
